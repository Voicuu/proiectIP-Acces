...
const DataScreen = ({ route }: { route: any }) => {
  const [data, setData] = useState<Person[]>([]); 
  const { name } = route.params; // Access the parameter
  ...
  useEffect(() => {
    ...
    onValue(dataRef, (snapshot) => {
      ...
      console.log(dataArray); // Log the data to check its value
      const filteredData = dataArray.filter((person: Person) => person.Nume === name); // Filter the data
